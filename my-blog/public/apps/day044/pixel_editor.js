...
rtl.module("program",["System","JS","Web","SysUtils","Math"],function () {
  "use strict";
  var $mod = this;
  this.TColorArr$clone = function (a) {
    var b = [];
    b.length = 16;
    for (var c = 0; c < 16; c++) b[c] = a[c].slice(0);
    return b;
  };
  this.Grid = rtl.arraySetLength(null,"",16,16);
  this.Canvas = null;
  this.Ctx = null;
  this.Picker = null;
  this.GRID_SIZE = 16;
  ...
  this.Render = function () {
    ...
    for (y = 0; y <= 15; y++) for (x = 0; x <= 15; x++) {
      $mod.Ctx.fillStyle = $mod.Grid[x][y];
      $mod.Ctx.fillRect(x * 20,y * 20,20,20);
      $mod.Ctx.strokeStyle = "#444444";
      $mod.Ctx.strokeRect(x * 20,y * 20,20,20);
    };
  };
  this.AutoSave = function () {
    window.localStorage.setItem("pixel_data",JSON.stringify($mod.TColorArr$clone($mod.Grid)));
  };
  this.OnCanvasClick = function (Event) {
    var Result = false;
    var ix = 0;
    var iy = 0;
    ix = pas.Math.Floor(Event.offsetX / 20);
    iy = pas.Math.Floor(Event.offsetY / 20);
    if ((ix in rtl.createSet(null,0,15)) && (iy in rtl.createSet(null,0,15))) {
      $mod.Grid[ix][iy] = $mod.Picker.value;
      $mod.Render();
      $mod.AutoSave();
    };
    Result = true;
  ...
  this.OnDownload = function (Event) {
    ...
    Link.href = $mod.Canvas.toDataURL("image/png");
    Link.click();
    Result = true;
    return Result;
  };
  this.OnClear = function (Event) {
    ...
    var x = 0;
    var y = 0;
    for (x = 0; x <= 15; x++) for (y = 0; y <= 15; y++) $mod.Grid[x][y] = "#000000";
    $mod.Render();
    $mod.AutoSave();
    Result = true;
    return Result;
  ...
  this.Init = function () {
    ...
    var x = 0;
    var y = 0;
    $mod.Canvas = document.getElementById("editor");
    $mod.Ctx = $mod.Canvas.getContext("2d");
    $mod.Picker = document.getElementById("picker");
    $mod.Canvas.onclick = rtl.createSafeCallback($mod,"OnCanvasClick");
    document.getElementById("btnDownload").onclick = rtl.createSafeCallback($mod,"OnDownload");
    document.getElementById("btnClear").onclick = rtl.createSafeCallback($mod,"OnClear");
    Data = window.localStorage.getItem("pixel_data");
    if (Data !== "") {
      $mod.Grid = JSON.parse(Data)}
     else {
      for (x = 0; x <= 15; x++) for (y = 0; y <= 15; y++) $mod.Grid[x][y] = "#000000";
    };
    $mod.Render();
