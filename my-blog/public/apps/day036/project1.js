...
rtl.module("program",["System","BrowserApp","JS","Web","SysUtils"],function () {
  "use strict";
  var $mod = this;
  this.StartDateInput = null;
  this.DisplaySpan = null;
  this.CalculateDays = function (Event) {
    var StartDate = 0.0;
    var Today = 0.0;
    var Diff = 0;
    var S = "";
    ...
    var d = 0;
    try {
      S = $mod.StartDateInput.value;
      y = pas.SysUtils.StrToInt(pas.System.Copy(S,1,4));
      m = pas.SysUtils.StrToInt(pas.System.Copy(S,6,2));
      d = pas.SysUtils.StrToInt(pas.System.Copy(S,9,2));
      StartDate = pas.SysUtils.EncodeDate(y,m,d);
      Today = pas.SysUtils.Date();
      Diff = pas.System.Trunc(Today - StartDate) + 1;
      $mod.DisplaySpan.innerText = pas.SysUtils.IntToStr(Diff);
    } catch ($e) {
      $mod.DisplaySpan.innerText = "--";
    };
  };
  this.Initialize = function () {
    $mod.StartDateInput = document.getElementById("start-date");
    $mod.DisplaySpan = document.getElementById("day-count");
    $mod.StartDateInput.value = pas.SysUtils.FormatDateTime("YYYY",pas.SysUtils.Date()) + "-01-01";
    $mod.StartDateInput.addEventListener("change",rtl.createSafeCallback($mod,"CalculateDays"));
    $mod.CalculateDays(null);
  };
  $mod.$main = function () {
    window.addEventListener("load",$mod.Initialize);
  };
});
