...
rtl.module("program",["System","Web","SysUtils","JS"],function () {
  ...
  this.ParseMarkdown = function (src) {
    var Result = "";
    var Lines = [];
    var i = 0;
    var Line = "";
    ...
    for (var $l = 0, $end = rtl.length(Lines) - 1; $l <= $end; $l++) {
      ...
      if (Line === "---") {
        ...
        if (pas.System.Copy(Line,1,2) === "# ") {
          Line = "<h1>" + pas.System.Copy(Line,3,Line.length) + "</h1>"}
         else if (pas.System.Copy(Line,1,3) === "## ") Line = "<h2>" + pas.System.Copy(Line,4,Line.length) + "</h2>";
      } else if ((Line.length >= 2) && ((Line.charAt(0) === "*") || (Line.charAt(0) === "-")) && (Line.charAt(1) === " ")) {
        Line = "<li>" + pas.System.Copy(Line,3,Line.length) + "</li>";
      };
      Line = Line.replace(new RegExp("\\*\\*(.*?)\\*\\*","g"),"<strong>$1</strong>");
      Line = Line.replace(new RegExp("\\[(.*?)\\]\\((.*?)\\)","g"),'<a href="$2" target="_blank">$1</a>');
      if ((pas.System.Pos("<",Line) !== 1) && (pas.SysUtils.Trim(Line) !== "")) Line = "<p>" + Line + "</p>";
      Result = Result + Line + "\n";
    };
    return Result;
  };
  this.DoUpdate = function (Event) {
    var Result = false;
    if (($mod.EditorElem != null) && ($mod.PreviewElem != null)) $mod.PreviewElem.innerHTML = $mod.ParseMarkdown($mod.EditorElem.value);
    Result = true;
    return Result;
  };
  $mod.$main = function () {
    $mod.EditorElem = document.getElementById("editor");
    $mod.PreviewElem = document.getElementById("preview");
    if ($mod.EditorElem != null) $mod.EditorElem.oninput = rtl.createSafeCallback($mod,"DoUpdate");
  };
});
