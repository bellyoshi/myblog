...
rtl.module("program",["System","Web","SysUtils","JS"],function () {
  ...
  this.PeekChar = function () {
    var Result = "\x00";
    if ($mod.Pos <= $mod.Expr.length) {
      Result = $mod.Expr.charAt($mod.Pos - 1)}
     else Result = "\x00";
    return Result;
  };
  this.GetChar = function () {
    var Result = "\x00";
    Result = $mod.PeekChar();
    if (Result !== "\x00") $mod.Pos += 1;
    return Result;
  };
  this.SkipWhite = function () {
    while ($mod.PeekChar() === " ") $mod.GetChar();
  };
  this.ParseFactor = function () {
    ...
    if ($mod.PeekChar() === "(") {
      $mod.GetChar();
      Result = $mod.ParseExpression();
      $mod.SkipWhite();
      if ($mod.PeekChar() === ")") $mod.GetChar();
    } else {
      S = "";
      while ($mod.PeekChar().charCodeAt() in rtl.createSet(null,48,57,46)) S = S + $mod.GetChar();
      if (S === "") {
        Result = 0}
       else Result = pas.SysUtils.StrToFloat(S);
    };
    return Result;
  };
  this.ParseTerm = function () {
    ...
    $mod.SkipWhite();
    while ($mod.PeekChar().charCodeAt() in rtl.createSet(42,47)) {
      Op = $mod.GetChar();
      NextVal = $mod.ParseFactor();
      if (Op === "*") {
        Result = Result * NextVal}
       else if (NextVal !== 0) Result = Result / NextVal;
      $mod.SkipWhite();
    };
    return Result;
  };
  this.ParseExpression = function () {
    ...
    var Op = "\x00";
    var NextVal = 0.0;
    Result = $mod.ParseTerm();
    $mod.SkipWhite();
    while ($mod.PeekChar().charCodeAt() in rtl.createSet(43,45)) {
      Op = $mod.GetChar();
      NextVal = $mod.ParseTerm();
      if (Op === "+") {
        Result = Result + NextVal}
  ...
  this.DoCalculate = function (Event) {
    var Result = false;
    var FinalResult = 0.0;
    Result = false;
    $mod.Expr = $mod.InputBox.value;
    $mod.Pos = 1;
    try {
      FinalResult = $mod.ParseExpression();
      $mod.ResultDiv.innerHTML = "結果: " + pas.SysUtils.FloatToStr(FinalResult);
    } catch ($e) {
      $mod.ResultDiv.innerHTML = "エラー: 計算できません";
    };
    return Result;
  };
  $mod.$main = function () {
    $mod.InputBox = document.getElementById("formula");
    $mod.CalcButton = document.getElementById("btnCalc");
    $mod.ResultDiv = document.getElementById("result");
    if ($mod.CalcButton != null) $mod.CalcButton.onclick = rtl.createSafeCallback($mod,"DoCalculate");
  };
});
