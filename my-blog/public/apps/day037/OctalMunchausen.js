...
rtl.module("program",["System","SysUtils","Web"],function () {
  "use strict";
  var $mod = this;
  this.BASE = 8;
  this.TARGET_DIGITS = 8;
  this.PowCache = rtl.arraySetLength(null,0,8);
  this.OutputDiv = null;
  this.AppendHtml = function (s) {
    $mod.OutputDiv.innerHTML = $mod.OutputDiv.innerHTML + s;
  };
  this.AppendLine = function (s) {
    $mod.AppendHtml('<span class="console-text">' + s + "</span><br>");
  };
  this.InitPowCache = function () {
    var i = 0;
    var j = 0;
    var p = 0;
    $mod.PowCache[0] = 1;
  ...
  this.CheckAndPrint = function (n) {
    var temp = 0;
    var digits = rtl.arraySetLength(null,0,8);
    var sum = 0;
    var i = 0;
    var sOctal = "";
    var sFormulaHtml = "";
    ...
    for (i = 0; i <= 7; i++) {
      digits[8 - 1 - i] = temp % 8;
      sum = sum + $mod.PowCache[temp % 8];
      temp = rtl.trunc(temp / 8);
    };
    if (sum === n) {
      sOctal = "";
      sFormulaHtml = "";
      for (i = 0; i <= 7; i++) {
        sOctal = sOctal + pas.SysUtils.IntToStr(digits[i]);
        if (sFormulaHtml !== "") sFormulaHtml = sFormulaHtml + '<span class="formula-eq"> + </span>';
        sFormulaHtml = sFormulaHtml + '<span class="formula-term">' + pas.SysUtils.IntToStr(digits[i]) + "<sup>" + pas.SysUtils.IntToStr(digits[i]) + "</sup></span>";
      };
      sFormulaHtml = sFormulaHtml + '<span class="formula-eq"> = </span><span class="formula-result">' + pas.SysUtils.IntToStr(n) + "</span>";
      $mod.AppendHtml('<div class="result-card"><div class="octal">' + sOctal + '</div><div class="decimal">10進数: ' + pas.SysUtils.IntToStr(n) + '</div><div class="formula-line">' + sFormulaHtml + "</div></div>");
    };
  };
  this.i = 0;
  this.Limit = 0;
  $mod.$main = function () {
    $mod.InitPowCache();
    $mod.Limit = 1;
    for ($mod.i = 1; $mod.i <= 8; $mod.i++) $mod.Limit = $mod.Limit * 8;
    $mod.Limit = $mod.Limit - 1;
    $mod.OutputDiv = document.getElementById("output");
    $mod.AppendLine("8進数 8桁固定探索開始 (0^0=1)");
    $mod.AppendLine("範囲: 00000000 \uFF5E 77777777");
    $mod.AppendLine("--------------------------------------------------");
    for (var $l = 0, $end = $mod.Limit; $l <= $end; $l++) {
      $mod.i = $l;
      $mod.CheckAndPrint($mod.i);
    };
    $mod.AppendLine("探索終了");
  };
});
