...
rtl.module("program",["System","Web","SysUtils"],function () {
  "use strict";
  var $mod = this;
  this.TInterest = {"0": "itTech", itTech: 0, "1": "itArt", itArt: 1, "2": "itSports", itSports: 2, "3": "itMusic", itMusic: 3};
  rtl.recNewT(this,"TUserRegistration",function () {
    this.UserName = "";
    this.Age = 0;
    this.$new = function () {
      var r = Object.create(this);
    ...
    this.$eq = function (b) {
      return (this.UserName === b.UserName) && (this.Age === b.Age) && rtl.eqSet(this.Interests,b.Interests);
    };
    this.$assign = function (s) {
      this.UserName = s.UserName;
      this.Age = s.Age;
      this.Interests = rtl.refSet(s.Interests);
      return this;
    };
  });
  rtl.recNewT(this,"TValidationResult",function () {
    this.IsValid = false;
    this.ErrorMessage = "";
    this.$eq = function (b) {
      return (this.IsValid === b.IsValid) && (this.ErrorMessage === b.ErrorMessage);
    };
    this.$assign = function (s) {
  ...
  this.ValidateUser = function (Data) {
    var Result = $mod.TValidationResult.$new();
    Result.IsValid = false;
    if (Data.UserName.length < 3) {
      Result.ErrorMessage = "ユーザー名は3文字以上で入力してください。";
      return Result;
    };
    if (!(Data.Age in rtl.createSet(null,18,120))) {
      Result.ErrorMessage = "年齢は18歳から120歳の間で入力してください。";
      return Result;
    };
    if (rtl.eqSet(Data.Interests,{})) {
  ...
  this.OnSubmitClick = function (_Event) {
    var Data = $mod.TUserRegistration.$new();
    var Result = $mod.TValidationResult.$new();
    var Doc = null;
    var ErrorDiv = null;
    var I = 0;
    ...
    var CB = null;
    Doc = window.document;
    ErrorDiv = Doc.getElementById("errorMessage");
    Data.UserName = Doc.getElementById("userName").value;
    Data.Age = pas.SysUtils.StrToIntDef(Doc.getElementById("age").value,0);
    Data.Interests = {};
    CBs = Doc.querySelectorAll('input[name="interest"]:checked');
    for (var $l = 0, $end = CBs.length - 1; $l <= $end; $l++) {
      I = $l;
      CB = CBs.item(I);
      if (CB.value === "itTech") Data.Interests = rtl.includeSet(Data.Interests,$mod.TInterest.itTech);
      if (CB.value === "itArt") Data.Interests = rtl.includeSet(Data.Interests,$mod.TInterest.itArt);
      if (CB.value === "itSports") Data.Interests = rtl.includeSet(Data.Interests,$mod.TInterest.itSports);
      if (CB.value === "itMusic") Data.Interests = rtl.includeSet(Data.Interests,$mod.TInterest.itMusic);
    };
    Result.$assign($mod.ValidateUser(Data));
    if (Result.IsValid) {
  ...
  $mod.$main = function () {
    window.document.getElementById("submitBtn").addEventListener("click",rtl.createSafeCallback($mod,"OnSubmitClick"));
  };
});
