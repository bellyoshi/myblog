...
rtl.module("program",["System","browserconsole","Web","SysUtils","Classes"],function () {
  "use strict";
  var $mod = this;
  this.ItemInput = null;
  this.AddBtn = null;
  this.ClearBtn = null;
  this.ItemListUI = null;
  this.DataList = null;
  this.STORAGE_KEY = "pas2js_shopping_data";
  this.LoadData = function () {
    var SavedData = "";
    $mod.DataList.Clear();
    SavedData = window.localStorage.getItem($mod.STORAGE_KEY);
    if (SavedData !== "") $mod.DataList.SetCommaText(SavedData);
  };
  this.SaveData = function () {
    window.localStorage.setItem($mod.STORAGE_KEY,$mod.DataList.GetCommaText());
  };
  this.RenderList = function () {
    var i = 0;
    var li = null;
    $mod.ItemListUI.innerHTML = "";
    for (var $l = 0, $end = $mod.DataList.GetCount() - 1; $l <= $end; $l++) {
      i = $l;
      li = document.createElement("li");
      li.innerHTML = pas.SysUtils.Format('%s <span class="delete-item" data-index="%d">×</span>',pas.System.VarRecs(18,$mod.DataList.Get(i),0,i));
      $mod.ItemListUI.appendChild(li);
    };
  };
  this.AddItem = function (Event) {
    var Result = false;
    if ($mod.ItemInput.value !== "") {
      $mod.DataList.Add($mod.ItemInput.value);
      $mod.ItemInput.value = "";
      $mod.SaveData();
      $mod.RenderList();
    };
  ...
  this.HandleListClick = function (Event) {
    var Result = false;
    var Idx = 0;
    if (Event.target.className === "delete-item") {
      Idx = pas.SysUtils.StrToInt(Event.target.getAttribute("data-index"));
      $mod.DataList.Delete(Idx);
      $mod.SaveData();
      $mod.RenderList();
    };
    Result = false;
  ...
  this.ClearAll = function (Event) {
    var Result = false;
    if (window.confirm("リストを空にしますか\uFF1F")) {
      $mod.DataList.Clear();
      $mod.SaveData();
      $mod.RenderList();
    };
  ...
  $mod.$main = function () {
    $mod.DataList = pas.Classes.TStringList.$create("Create$1");
    $mod.ItemInput = document.getElementById("itemInput");
    $mod.AddBtn = document.getElementById("addBtn");
    $mod.ClearBtn = document.getElementById("clearBtn");
    $mod.ItemListUI = document.getElementById("itemList");
    $mod.AddBtn.onclick = rtl.createSafeCallback($mod,"AddItem");
    $mod.ClearBtn.onclick = rtl.createSafeCallback($mod,"ClearAll");
    $mod.ItemListUI.onclick = rtl.createSafeCallback($mod,"HandleListClick");
    $mod.LoadData();
    $mod.RenderList();
  };
});
