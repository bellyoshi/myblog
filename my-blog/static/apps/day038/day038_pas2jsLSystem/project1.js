...
rtl.module("RTLConsts",["System"],function () {
  "use strict";
  var $mod = this;
  $mod.$resourcestrings = {SArgumentMissing: {org: 'Missing argument in format "%s"'}, SInvalidFormat: {org: 'Invalid format specifier : "%s"'}, SInvalidArgIndex: {org: 'Invalid argument index in format: "%s"'}, SErrInvalidInteger: {org: 'Invalid integer value: "%s"'}};
});
...
rtl.module("program",["System","SysUtils","Web","JS"],function () {
  "use strict";
  var $mod = this;
  rtl.recNewT(this,"TState",function () {
    this.X = 0.0;
    this.Y = 0.0;
    this.Angle = 0.0;
    this.LineWidth = 0.0;
    this.$eq = function (b) {
      return (this.X === b.X) && (this.Y === b.Y) && (this.Angle === b.Angle) && (this.LineWidth === b.LineWidth);
    };
    this.$assign = function (s) {
      this.X = s.X;
      this.Y = s.Y;
  ...
  rtl.recNewT(this,"TLSystemPreset",function () {
    this.Axiom = "";
    this.RuleF = "";
    this.RuleX = "";
    this.Angle = 0.0;
    ...
    this.InitialY = 0.0;
    this.$eq = function (b) {
      return (this.Axiom === b.Axiom) && (this.RuleF === b.RuleF) && (this.RuleX === b.RuleX) && (this.Angle === b.Angle) && (this.InitialStep === b.InitialStep) && (this.InitialY === b.InitialY);
    };
    this.$assign = function (s) {
      this.Axiom = s.Axiom;
      this.RuleF = s.RuleF;
  ...
  this.Ctx = null;
  this.CurrentString = "";
  this.CurrentPreset = this.TLSystemPreset.$new();
  this.Depth = 3;
  this.SetPreset = function (Kind) {
    if (Kind === "tree") {
      $mod.CurrentPreset.Axiom = "X";
      $mod.CurrentPreset.RuleF = "FF";
      $mod.CurrentPreset.RuleX = "F[[+X]-X]-F[-FX]+X";
      $mod.CurrentPreset.Angle = 25.0;
      $mod.CurrentPreset.InitialStep = 4.0;
      $mod.CurrentPreset.InitialY = 580;
    } else if (Kind === "fern") {
      $mod.CurrentPreset.Axiom = "X";
      $mod.CurrentPreset.RuleF = "FF";
      $mod.CurrentPreset.RuleX = "F+[[X]-X]-F[-FX]+X";
      $mod.CurrentPreset.Angle = 20.0;
      $mod.CurrentPreset.InitialStep = 3.5;
      $mod.CurrentPreset.InitialY = 580;
    } else if (Kind === "snowflake") {
      $mod.CurrentPreset.Axiom = "F++F++F";
      $mod.CurrentPreset.RuleF = "F-F++F-F";
      $mod.CurrentPreset.RuleX = "";
      $mod.CurrentPreset.Angle = 60.0;
      $mod.CurrentPreset.InitialStep = 5.0;
      $mod.CurrentPreset.InitialY = 450;
    };
  };
  this.ApplyRules = function () {
    ...
    var C = "\x00";
    NextStr = "";
    for (var $l = 1, $end = $mod.CurrentString.length; $l <= $end; $l++) {
      i = $l;
      C = $mod.CurrentString.charAt(i - 1);
      var $tmp = C;
      if ($tmp === "F") {
        NextStr = NextStr + $mod.CurrentPreset.RuleF}
       else if ($tmp === "X") {
        NextStr = NextStr + $mod.CurrentPreset.RuleX}
       else {
        NextStr = NextStr + C;
      };
    };
    $mod.CurrentString = NextStr;
  };
  this.Render = function () {
    var i = 0;
    var Stack = [];
    var Current = $mod.TState.$new();
    var NewX = 0.0;
    var NewY = 0.0;
    var Progress = 0.0;
    $mod.Ctx.clearRect(0,0,$mod.Canvas.width,$mod.Canvas.height);
    $mod.CurrentString = $mod.CurrentPreset.Axiom;
    for (var $l = 1, $end = $mod.Depth; $l <= $end; $l++) {
      i = $l;
      $mod.ApplyRules();
    };
    Current.X = $mod.Canvas.width / 2;
    Current.Y = $mod.CurrentPreset.InitialY;
    Current.Angle = -90;
    Current.LineWidth = ($mod.Depth * 1.5) + 1;
    $mod.Ctx.beginPath();
    $mod.Ctx.lineCap = "round";
    for (var $l1 = 1, $end1 = $mod.CurrentString.length; $l1 <= $end1; $l1++) {
      i = $l1;
      var $tmp = $mod.CurrentString.charAt(i - 1);
      if ($tmp === "F") {
        NewX = Current.X + (Math.cos((Current.Angle * Math.PI) / 180) * $mod.CurrentPreset.InitialStep);
        NewY = Current.Y + (Math.sin((Current.Angle * Math.PI) / 180) * $mod.CurrentPreset.InitialStep);
        $mod.Ctx.beginPath();
        $mod.Ctx.moveTo(Current.X,Current.Y);
        $mod.Ctx.lineTo(NewX,NewY);
        Progress = i / $mod.CurrentString.length;
        $mod.Ctx.lineWidth = Current.LineWidth;
        if ($mod.CurrentPreset.RuleX !== "") {
          $mod.Ctx.strokeStyle = "rgb(" + pas.SysUtils.IntToStr(Math.round(100 * (1 - Progress))) + ", " + pas.SysUtils.IntToStr(Math.round(100 + (155 * Progress))) + ", 50)"}
         else $mod.Ctx.strokeStyle = "#333";
        $mod.Ctx.stroke();
        Current.X = NewX;
        Current.Y = NewY;
      } else if ($tmp === "+") {
        Current.Angle = Current.Angle + $mod.CurrentPreset.Angle}
       else if ($tmp === "-") {
        Current.Angle = Current.Angle - $mod.CurrentPreset.Angle}
       else if ($tmp === "[") {
        Stack = rtl.arraySetLength(Stack,$mod.TState,rtl.length(Stack) + 1);
        Stack[rtl.length(Stack) - 1].$assign(Current);
        Current.LineWidth = Current.LineWidth * 0.7;
      } else if ($tmp === "]") {
        Current.$assign(Stack[rtl.length(Stack) - 1]);
        Stack = rtl.arraySetLength(Stack,$mod.TState,rtl.length(Stack) - 1);
      };
    };
    document.getElementById("status").innerHTML = "生成文字列長: " + pas.SysUtils.IntToStr($mod.CurrentString.length);
  };
  this.HandleInput = function (Event) {
    var Result = false;
    $mod.Depth = pas.SysUtils.StrToInt(document.getElementById("depthRange").value);
    document.getElementById("depthVal").innerHTML = pas.SysUtils.IntToStr($mod.Depth);
    $mod.Render();
    Result = true;
    return Result;
  };
  this.HandleSelect = function (Event) {
    var Result = false;
    $mod.SetPreset(Event.target.value);
    $mod.Render();
    Result = true;
    return Result;
  };
  $mod.$main = function () {
    $mod.Canvas = document.getElementById("mainCanvas");
    $mod.Ctx = $mod.Canvas.getContext("2d");
    document.getElementById("depthRange").oninput = rtl.createSafeCallback($mod,"HandleInput");
    document.getElementById("presetSelect").onchange = rtl.createSafeCallback($mod,"HandleSelect");
    $mod.SetPreset("tree");
    $mod.Render();
  };
});
