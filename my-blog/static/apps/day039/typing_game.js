...
rtl.module("program",["System","JS","SysUtils","Types","Web"],function () {
  "use strict";
  var $mod = this;
  this.Canvas = null;
  this.Ctx = null;
  ...
  this.UserInput = "";
  this.CheckTimer = 0;
  rtl.createClass(this,"TWord",pas.System.TObject,function () {
    this.$init = function () {
  ...
  this.SpawnWord = function () {
    $mod.ActiveWords.push($mod.TWord.$create("Create$1",[$mod.Keywords[pas.System.Random(rtl.length($mod.Keywords))]]));
  };
  this.Update = function (HighResTime) {
    var i = 0;
    var W = null;
    $mod.Ctx.fillStyle = "#2c3e50";
    $mod.Ctx.fillRect(0,0,$mod.Canvas.width,$mod.Canvas.height);
    $mod.Ctx.font = "24px Monospace";
    $mod.Ctx.fillStyle = "#ecf0f1";
    for (var $l = $mod.ActiveWords.length - 1; $l >= 0; $l--) {
      i = $l;
      W = rtl.getObject($mod.ActiveWords[i]);
      W.X = W.X + W.Speed;
      $mod.Ctx.fillText(W.Text,W.X,W.Y);
      if (W.X > $mod.Canvas.width) $mod.ActiveWords.splice(i,1);
    };
    $mod.Ctx.fillStyle = "#f1c40f";
    $mod.Ctx.font = "20px Monospace";
    $mod.Ctx.fillText("入力: " + $mod.UserInput + "|",20,$mod.Canvas.height - 20);
    window.requestAnimationFrame($mod.Update);
  };
  this.HandleKeyDown = function (Event) {
    ...
    if (Event.key === "Backspace") {
      if ($mod.UserInput.length > 0) $mod.UserInput = pas.System.Copy($mod.UserInput,1,$mod.UserInput.length - 1);
      El = document.getElementById("current-word");
      if ($mod.UserInput === "") {
        El.textContent = "_"}
       else El.textContent = $mod.UserInput;
    } else if (Event.key === "Enter") {
      for (var $l = $mod.ActiveWords.length - 1; $l >= 0; $l--) {
        i = $l;
        W = rtl.getObject($mod.ActiveWords[i]);
        if (W.Text === $mod.UserInput) {
          $mod.ActiveWords.splice(i,1);
          $mod.Score = $mod.Score + W.Text.length;
          break;
        };
      };
      $mod.UserInput = "";
      document.getElementById("current-word").textContent = "_";
      document.getElementById("score-value").textContent = pas.SysUtils.IntToStr($mod.Score);
    } else if ((Event.key.length === 1) && !Event.ctrlKey && !Event.altKey && !Event.metaKey) {
      $mod.UserInput = $mod.UserInput + Event.key;
      El = document.getElementById("current-word");
      El.textContent = $mod.UserInput;
    };
    Result = true;
    return Result;
  };
  this.StartGame = function () {
    $mod.ActiveWords = new Array();
    $mod.Canvas = document.getElementById("gameCanvas");
    $mod.Ctx = $mod.Canvas.getContext("2d");
    window.setInterval(rtl.createSafeCallback($mod,"SpawnWord"),2000);
    window.requestAnimationFrame($mod.Update);
    window.addEventListener("keydown",$mod.HandleKeyDown);
  };
  this.GetKeywordsReady = function () {
    var Result = false;
    return !!window._keywordsReady;
    return Result;
  };
  this.CheckKeywordsAndStart = function () {
    if ($mod.GetKeywordsReady()) {
      window.clearInterval($mod.CheckTimer);
      $mod.Keywords = window["_keywordsData"];
      if (rtl.length($mod.Keywords) === 0) $mod.Keywords = ["absolute","and","array","asm","begin","break","case","const","constructor","continue","destructor","div","do","downto","else","end","file","for","function","goto","if","implementation","in","inherited","inline","interface","label","mod","nil","not","object","of","on","operator","or","packed","procedure","program","record","reintroduce","repeat","self","set","shl","shr","string","then","to","type","unit","until","uses","var","while","with","xor","as","class","except","exports","finalization","finally","initialization","is","library","property","raise","threadvar","try","dispose","exit","false","new","true","abs","arctan","boolean","char","cos","eof","eoln","exp","input","integer","ln","maxint","odd","ord","output","pack","page","pred","read","readln","real","reset","rewrite","round","sin","sqr","sqrt","succ","text","trunc","write","writeln"];
      $mod.StartGame();
    };
  };
  $mod.$main = function () {
    ...
    $mod.CheckTimer = window.setInterval(rtl.createSafeCallback($mod,"CheckKeywordsAndStart"),50);
  };
});
