...
rtl.module("program",["System","browserconsole","Web","SysUtils","DateUtils"],function () {
  "use strict";
  var $mod = this;
  this.CurrentDate = 0.0;
  this.RenderCalendar = function () {
    ...
    var Month = 0;
    var Day = 0;
    var FirstDayOfMonth = 0.0;
    var DaysInMonth = 0;
    var StartDay = 0;
    var d = 0;
    var i = 0;
    pas.SysUtils.DecodeDate($mod.CurrentDate,{get: function () {
        return Year;
      }, set: function (v) {
        Year = v;
      }},{get: function () {
    ...
    BtnPrev.onclick = function (Event) {
      var Result = false;
      $mod.CurrentDate = pas.SysUtils.IncMonth($mod.CurrentDate,-1);
      $mod.RenderCalendar();
      Result = false;
      return Result;
    };
    Title = document.createElement("span");
    Title.innerText = pas.SysUtils.Format(" %d年 %d月 ",pas.System.VarRecs(0,Year,0,Month));
    ...
    FirstDayOfMonth = pas.SysUtils.EncodeDate(Year,Month,1);
    DaysInMonth = pas.DateUtils.DaysInAMonth(Year,Month);
    StartDay = pas.SysUtils.DayOfWeek(FirstDayOfMonth);
    Row = document.createElement("tr");
    for (i = 1; i <= 7; i++) {
      Cell = document.createElement("th");
      Cell.innerText = pas.SysUtils.FormatSettings.ShortDayNames[i - 1];
    ...
    Row = document.createElement("tr");
    for (var $l = 1, $end = StartDay - 1; $l <= $end; $l++) {
      i = $l;
      Cell = document.createElement("td");
      Cell.style.setProperty("border","1px solid #ccc");
      Row.appendChild(Cell);
    };
    for (var $l1 = 1, $end1 = DaysInMonth; $l1 <= $end1; $l1++) {
      d = $l1;
      Cell = document.createElement("td");
      Cell.innerText = pas.SysUtils.IntToStr(d);
      Cell.style.setProperty("border","1px solid #ccc");
      Cell.style.setProperty("padding","10px");
      Cell.style.setProperty("text-align","center");
      Row.appendChild(Cell);
      if (((((d + StartDay) - 1) % 7) === 0) && (d < DaysInMonth)) {
        Table.appendChild(Row);
        Row = document.createElement("tr");
      };
    };
  ...
  $mod.$main = function () {
    $mod.CurrentDate = pas.SysUtils.Now();
    $mod.RenderCalendar();
  };
});
