...
rtl.module("program",["System","Web","SysUtils","JS"],function () {
  "use strict";
  var $mod = this;
  this.TInputState = {"0": "isSelectFirst", isSelectFirst: 0, "1": "isSelectOp", isSelectOp: 1, "2": "isSelectSecond", isSelectSecond: 2, "3": "isGameOver", isGameOver: 3, "4": "isWin", isWin: 4};
  this.Numbers = [];
  this.SelectedIdx = -1;
  this.SelectedOp = "";
  this.State = this.TInputState.isSelectFirst;
  this.CanSolve = function (Nums) {
    var Result = false;
    var i = 0;
    var j = 0;
    var k = 0;
    ...
    var NextNums = [];
    var Res = 0.0;
    if (rtl.length(Nums) === 1) return Math.abs(Nums[0] - 24) < 0.0001;
    for (var $l = 0, $end = rtl.length(Nums) - 1; $l <= $end; $l++) {
      i = $l;
      for (var $l1 = 0, $end1 = rtl.length(Nums) - 1; $l1 <= $end1; $l1++) {
        j = $l1;
        if (i !== j) {
          NextNums = rtl.arraySetLength(NextNums,0.0,rtl.length(Nums) - 1);
          k = 0;
          for (var $l2 = 0, $end2 = rtl.length(Nums) - 1; $l2 <= $end2; $l2++) {
            m = $l2;
            if ((m !== i) && (m !== j)) {
              NextNums[k] = Nums[m];
              k += 1;
          ...
          for (op = 1; op <= 4; op++) {
            ...
            if ($tmp === 1) {
              ...
              Res = Nums[i] * Nums[j]}
             else if ($tmp === 4) if (Nums[j] !== 0) {
              Res = Nums[i] / Nums[j]}
             else continue;
            NextNums[rtl.length(NextNums) - 1] = Res;
            if ($mod.CanSolve(rtl.arrayRef(NextNums))) return true;
          };
        };
      };
    };
  ...
  this.HandleNumberBtnClick = function (Event) {
    var Result = false;
    var Btn = null;
    var Idx = 0;
    Btn = Event.target;
    Idx = pas.SysUtils.StrToInt(Btn.getAttribute("data-idx"));
    $mod.HandleNumberClick(Idx);
    Result = true;
    return Result;
  };
  this.HandleNumberClick = function (Index) {
    var N1 = 0.0;
    var N2 = 0.0;
    var Res = 0.0;
    var NewArr = [];
    var i = 0;
    var $tmp = $mod.State;
    if ($tmp === $mod.TInputState.isSelectFirst) {
      $mod.SelectedIdx = Index;
      $mod.State = $mod.TInputState.isSelectOp;
    } else if ($tmp === $mod.TInputState.isSelectSecond) {
      if (Index === $mod.SelectedIdx) return;
      N1 = $mod.Numbers[$mod.SelectedIdx];
      N2 = $mod.Numbers[Index];
      if ($mod.SelectedOp === "+") {
        Res = N1 + N2}
       else if ($mod.SelectedOp === "-") {
        Res = N1 - N2}
       else if ($mod.SelectedOp === "*") {
        Res = N1 * N2}
       else if (($mod.SelectedOp === "/") && (N2 !== 0)) {
        Res = N1 / N2}
       else return;
      NewArr = rtl.arraySetLength(NewArr,0.0,0);
      for (var $l = 0, $end = rtl.length($mod.Numbers) - 1; $l <= $end; $l++) {
        i = $l;
        if ((i !== $mod.SelectedIdx) && (i !== Index)) {
          NewArr = rtl.arraySetLength(NewArr,0.0,rtl.length(NewArr) + 1);
          NewArr[rtl.length(NewArr) - 1] = $mod.Numbers[i];
        };
      };
      NewArr = rtl.arraySetLength(NewArr,0.0,rtl.length(NewArr) + 1);
      NewArr[rtl.length(NewArr) - 1] = Res;
      $mod.Numbers = rtl.arrayRef(NewArr);
      $mod.SelectedIdx = -1;
      $mod.SelectedOp = "";
      $mod.State = $mod.TInputState.isSelectFirst;
      if (rtl.length($mod.Numbers) === 1) {
        if (Math.abs($mod.Numbers[0] - 24) < 0.0001) {
          $mod.State = $mod.TInputState.isWin;
        } else {
          $mod.State = $mod.TInputState.isGameOver;
        };
      };
    };
    $mod.UpdateUI();
  };
  this.HandleOpClick = function (OpStr) {
    if ($mod.State === $mod.TInputState.isSelectOp) {
      $mod.SelectedOp = OpStr;
      $mod.State = $mod.TInputState.isSelectSecond;
      $mod.UpdateUI();
    };
  };
  this.UpdateUI = function () {
    ...
    var Msg = "";
    var i = 0;
    Container = document.getElementById("number-container");
    Container.innerHTML = "";
    for (var $l = 0, $end = rtl.length($mod.Numbers) - 1; $l <= $end; $l++) {
      i = $l;
      Btn = document.createElement("button");
      Btn.className = "num-btn";
      if (i === $mod.SelectedIdx) Btn.classList.add("selected");
      Btn.textContent = pas.SysUtils.FloatToStr($mod.Numbers[i]);
      Btn.setAttribute("data-idx",pas.SysUtils.IntToStr(i));
      Btn.onclick = rtl.createSafeCallback($mod,"HandleNumberBtnClick");
      Container.appendChild(Btn);
    };
    var $tmp = $mod.State;
    if ($tmp === $mod.TInputState.isSelectFirst) {
      Msg = "数字を選んでください"}
     else if ($tmp === $mod.TInputState.isSelectOp) {
      Msg = "演算子を選んでください"}
     else if ($tmp === $mod.TInputState.isSelectSecond) {
      Msg = "2つ目の数字を選んでください"}
     else if ($tmp === $mod.TInputState.isGameOver) {
      Msg = "残念、24になりませんでした"}
     else if ($tmp === $mod.TInputState.isWin) Msg = "Congratulations!おめでとう 🎉";
    document.getElementById("msg").innerHTML = Msg;
  };
  this.NewGame = function () {
    var i = 0;
    $mod.Numbers = rtl.arraySetLength($mod.Numbers,0.0,4);
    do {
      for (i = 0; i <= 3; i++) $mod.Numbers[i] = pas.System.Random(9) + 1;
    } while (!$mod.CanSolve(rtl.arrayRef($mod.Numbers)));
    $mod.SelectedIdx = -1;
    $mod.SelectedOp = "";
    $mod.State = $mod.TInputState.isSelectFirst;
    $mod.UpdateUI();
  ...
  $mod.$main = function () {
    ...
    window["game"]["HandleOpClick"] = $mod.HandleOpClick;
    window["game"]["NewGame"] = $mod.NewGame;
    $mod.NewGame();
  };
});
