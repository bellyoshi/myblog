...
rtl.module("program",["System","Web","JS","webaudio"],function () {
  "use strict";
  var $mod = this;
  rtl.recNewT(this,"TNote",function () {
    ...
    this.Len = 0.0;
    this.$eq = function (b) {
      return (this.Freq === b.Freq) && (this.Len === b.Len);
    };
  ...
  this.G = 392.00;
  this.A = 440.00;
  this.MELODY$a$clone = function (a) {
    var b = [];
    b.length = 32;
    for (var c = 0; c < 32; c++) b[c] = $mod.TNote.$clone(a[c]);
    return b;
  };
  this.MELODY = [this.TNote.$clone({Freq: 261.63, Len: 1.0}),this.TNote.$clone({Freq: 293.66, Len: 1.0}),this.TNote.$clone({Freq: 329.63, Len: 1.0}),this.TNote.$clone({Freq: 349.23, Len: 1.0}),this.TNote.$clone({Freq: 329.63, Len: 1.0}),this.TNote.$clone({Freq: 293.66, Len: 1.0}),this.TNote.$clone({Freq: 261.63, Len: 1.0}),this.TNote.$clone({Freq: 0, Len: 1.0}),this.TNote.$clone({Freq: 329.63, Len: 1.0}),this.TNote.$clone({Freq: 349.23, Len: 1.0}),this.TNote.$clone({Freq: 392, Len: 1.0}),this.TNote.$clone({Freq: 440, Len: 1.0}),this.TNote.$clone({Freq: 392, Len: 1.0}),this.TNote.$clone({Freq: 349.23, Len: 1.0}),this.TNote.$clone({Freq: 329.63, Len: 1.0}),this.TNote.$clone({Freq: 0, Len: 1.0}),this.TNote.$clone({Freq: 261.63, Len: 2.0}),this.TNote.$clone({Freq: 261.63, Len: 2.0}),this.TNote.$clone({Freq: 261.63, Len: 2.0}),this.TNote.$clone({Freq: 261.63, Len: 2.0}),this.TNote.$clone({Freq: 261.63, Len: 0.5}),this.TNote.$clone({Freq: 261.63, Len: 0.5}),this.TNote.$clone({Freq: 293.66, Len: 0.5}),this.TNote.$clone({Freq: 293.66, Len: 0.5}),this.TNote.$clone({Freq: 329.63, Len: 0.5}),this.TNote.$clone({Freq: 329.63, Len: 0.5}),this.TNote.$clone({Freq: 349.23, Len: 0.5}),this.TNote.$clone({Freq: 349.23, Len: 0.5}),this.TNote.$clone({Freq: 329.63, Len: 1.0}),this.TNote.$clone({Freq: 293.66, Len: 1.0}),this.TNote.$clone({Freq: 261.63, Len: 1.0}),this.TNote.$clone({Freq: 0, Len: 1.0})];
  this.PlayNote = function (Freq, StartTime, Duration, Vol) {
    var Osc = null;
    var Gain = null;
    Osc = $mod.AudioCtx.createOscillator();
    Gain = $mod.AudioCtx.createGain();
    Osc.connect(Gain);
    Gain.connect($mod.AudioCtx.destination);
    Osc.type = "triangle";
  ...
  this.PlayPart = function (StartTime, Vol) {
    var CurrentTime = 0.0;
    var Tempo = 0.0;
    var I = 0;
    Tempo = 0.4;
    ...
    for (I = 0; I <= 31; I++) {
      if ($mod.MELODY[I].Freq > 0) $mod.PlayNote($mod.MELODY[I].Freq,CurrentTime,$mod.MELODY[I].Len * Tempo,Vol);
      CurrentTime = CurrentTime + ($mod.MELODY[I].Len * Tempo);
    };
  };
  this.StartRinsho = function () {
    var BaseTime = 0.0;
    if ($mod.AudioCtx === null) $mod.AudioCtx = new AudioContext();
    if ($mod.AudioCtx.state === "suspended") $mod.AudioCtx.resume();
    BaseTime = $mod.AudioCtx.currentTime + 0.1;
    $mod.PlayPart(BaseTime,0.3);
    $mod.PlayPart(BaseTime + (8 * 0.4),0.2);
    $mod.PlayPart(BaseTime + (16 * 0.4),0.1);
  };
  $mod.$main = function () {
    window["StartRinsho"] = $mod.StartRinsho;
  };
});
