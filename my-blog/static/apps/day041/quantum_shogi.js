...
rtl.module("RTLConsts",["System"],function () {
  "use strict";
  var $mod = this;
  $mod.$resourcestrings = {SArgumentMissing: {org: 'Missing argument in format "%s"'}, SInvalidFormat: {org: 'Invalid format specifier : "%s"'}, SInvalidArgIndex: {org: 'Invalid argument index in format: "%s"'}, SListCapacityError: {org: "List capacity (%s) exceeded."}, SListIndexError: {org: "List index (%s) out of bounds"}, SErrInvalidInteger: {org: 'Invalid integer value: "%s"'}};
});
...
rtl.module("quantum_shogi_logic",["System","Classes"],function () {
  "use strict";
  var $mod = this;
  this.TPieceType = {"0": "ptPawn", ptPawn: 0, "1": "ptLance", ptLance: 1, "2": "ptKnight", ptKnight: 2, "3": "ptSilver", ptSilver: 3, "4": "ptGold", ptGold: 4, "5": "ptBishop", ptBishop: 5, "6": "ptRook", ptRook: 6, "7": "ptKing", ptKing: 7};
  rtl.createClass(this,"TQuantumPiece",pas.System.TObject,function () {
    this.$init = function () {
      pas.System.TObject.$init.call(this);
      this.Owner = 0;
      this.InitialGroup = 0;
    ...
    this.Create$1 = function (AOwner, AInitialGroup) {
      pas.System.TObject.Create.call(this);
      this.Owner = AOwner;
      this.InitialGroup = AInitialGroup;
      this.Possibilities = rtl.createSet($mod.TPieceType.ptPawn,$mod.TPieceType.ptLance,$mod.TPieceType.ptKnight,$mod.TPieceType.ptSilver,$mod.TPieceType.ptGold,$mod.TPieceType.ptBishop,$mod.TPieceType.ptRook,$mod.TPieceType.ptKing);
      this.IsPromoted = false;
      return this;
    };
  });
  rtl.createClass(this,"TGameLogic",pas.System.TObject,function () {
    this.Board$a$clone = function (a) {
      var b = [];
      b.length = 9;
    ...
    this.$init = function () {
      pas.System.TObject.$init.call(this);
      this.Board = rtl.arraySetLength(null,null,9,9);
      this.Stands = rtl.arraySetLength(null,null,2);
    };
    this.$final = function () {
    ...
    this.Create$1 = function () {
      ...
      var y = 0;
      pas.System.TObject.Create.call(this);
      this.Stands[0] = pas.Classes.TList.$create("Create$1");
      this.Stands[1] = pas.Classes.TList.$create("Create$1");
      for (y = 1; y <= 9; y++) for (x = 1; x <= 9; x++) this.Board[x - 1][y - 1] = null;
      this.InitBoard();
      return this;
    };
    this.InitBoard = function () {
      ...
      var y = 0;
      for (y = 1; y <= 9; y++) for (x = 1; x <= 9; x++) this.Board[x - 1][y - 1] = null;
      this.Board[0][8] = $mod.TQuantumPiece.$create("Create$1",[0,0]);
      this.Board[1][8] = $mod.TQuantumPiece.$create("Create$1",[0,0]);
      this.Board[2][8] = $mod.TQuantumPiece.$create("Create$1",[0,0]);
      this.Board[3][8] = $mod.TQuantumPiece.$create("Create$1",[0,0]);
      this.Board[4][8] = $mod.TQuantumPiece.$create("Create$1",[0,0]);
      this.Board[5][8] = $mod.TQuantumPiece.$create("Create$1",[0,0]);
      this.Board[6][8] = $mod.TQuantumPiece.$create("Create$1",[0,0]);
      this.Board[7][8] = $mod.TQuantumPiece.$create("Create$1",[0,0]);
      this.Board[8][8] = $mod.TQuantumPiece.$create("Create$1",[0,0]);
      this.Board[1][7] = $mod.TQuantumPiece.$create("Create$1",[0,0]);
      this.Board[7][7] = $mod.TQuantumPiece.$create("Create$1",[0,0]);
      for (x = 1; x <= 9; x++) this.Board[x - 1][6] = $mod.TQuantumPiece.$create("Create$1",[0,0]);
      this.Board[0][0] = $mod.TQuantumPiece.$create("Create$1",[1,1]);
      this.Board[1][0] = $mod.TQuantumPiece.$create("Create$1",[1,1]);
      this.Board[2][0] = $mod.TQuantumPiece.$create("Create$1",[1,1]);
      this.Board[3][0] = $mod.TQuantumPiece.$create("Create$1",[1,1]);
      this.Board[4][0] = $mod.TQuantumPiece.$create("Create$1",[1,1]);
      this.Board[5][0] = $mod.TQuantumPiece.$create("Create$1",[1,1]);
      this.Board[6][0] = $mod.TQuantumPiece.$create("Create$1",[1,1]);
      this.Board[7][0] = $mod.TQuantumPiece.$create("Create$1",[1,1]);
      this.Board[8][0] = $mod.TQuantumPiece.$create("Create$1",[1,1]);
      this.Board[1][1] = $mod.TQuantumPiece.$create("Create$1",[1,1]);
      this.Board[7][1] = $mod.TQuantumPiece.$create("Create$1",[1,1]);
      for (x = 1; x <= 9; x++) this.Board[x - 1][2] = $mod.TQuantumPiece.$create("Create$1",[1,1]);
    };
    this.InitialCountPerGroup = function (pt) {
      var Result = 0;
      var $tmp = pt;
      if ($tmp === $mod.TPieceType.ptPawn) {
        Result = 9}
       else if ($tmp === $mod.TPieceType.ptLance) {
        Result = 2}
       else if ($tmp === $mod.TPieceType.ptKnight) {
        Result = 2}
       else if ($tmp === $mod.TPieceType.ptSilver) {
        Result = 2}
       else if ($tmp === $mod.TPieceType.ptGold) {
        Result = 2}
       else if ($tmp === $mod.TPieceType.ptBishop) {
        Result = 1}
       else if ($tmp === $mod.TPieceType.ptRook) {
        Result = 1}
       else if ($tmp === $mod.TPieceType.ptKing) Result = 1;
      return Result;
    };
    this.CountPiecesWithTypeInGroup = function (AGroupId, pt) {
      var Result = 0;
      var x = 0;
      var y = 0;
      var s = 0;
      ...
      var p = null;
      Result = 0;
      for (y = 1; y <= 9; y++) for (x = 1; x <= 9; x++) if ((this.Board[x - 1][y - 1] !== null) && (this.Board[x - 1][y - 1].InitialGroup === AGroupId) && rtl.eqSet(this.Board[x - 1][y - 1].Possibilities,rtl.createSet(pt))) Result += 1;
      for (s = 0; s <= 1; s++) for (var $l = 0, $end = this.Stands[s].GetCount() - 1; $l <= $end; $l++) {
        i = $l;
        p = rtl.getObject(this.Stands[s].Get(i));
        if ((p.InitialGroup === AGroupId) && rtl.eqSet(p.Possibilities,rtl.createSet(pt))) Result += 1;
      };
      return Result;
    };
    this.WouldExceedTypeCount = function (AOwner, p, newSet) {
      var Result = false;
      var pt = 0;
      var cnt = 0;
      Result = false;
      for (var $l in newSet) {
        pt = +$l;
        if (rtl.neSet(newSet,rtl.createSet(pt))) continue;
        cnt = this.CountPiecesWithTypeInGroup(p.InitialGroup,pt);
        if (rtl.neSet(p.Possibilities,rtl.createSet(pt))) cnt += 1;
        if (cnt > this.InitialCountPerGroup(pt)) {
          Result = true;
          return Result;
        };
        return Result;
    ...
    this.RemovePossibilityFromGroup = function (AGroupId, pt, excludePiece) {
      var x = 0;
      var y = 0;
      var s = 0;
      var i = 0;
      var q = null;
      for (y = 1; y <= 9; y++) for (x = 1; x <= 9; x++) if ((this.Board[x - 1][y - 1] !== null) && (this.Board[x - 1][y - 1].InitialGroup === AGroupId) && (this.Board[x - 1][y - 1] !== excludePiece)) {
        q = this.Board[x - 1][y - 1];
        if ((pt in q.Possibilities) && rtl.neSet(q.Possibilities,rtl.createSet(pt))) q.Possibilities = rtl.diffSet(q.Possibilities,rtl.createSet(pt));
      };
      for (s = 0; s <= 1; s++) for (var $l = 0, $end = this.Stands[s].GetCount() - 1; $l <= $end; $l++) {
        i = $l;
        q = rtl.getObject(this.Stands[s].Get(i));
        if ((q.InitialGroup === AGroupId) && (q !== excludePiece) && (pt in q.Possibilities) && rtl.neSet(q.Possibilities,rtl.createSet(pt))) q.Possibilities = rtl.diffSet(q.Possibilities,rtl.createSet(pt));
      };
    };
    this.IsPathClear = function (fx, fy, tx, ty) {
      var Result = false;
      var x = 0;
      var y = 0;
      var dx = 0;
    ...
    this.IsMovePathBlocked = function (fx, fy, tx, ty, newSet) {
      var Result = false;
      if (rtl.eqSet(rtl.intersectSet(newSet,rtl.createSet($mod.TPieceType.ptRook,$mod.TPieceType.ptBishop,$mod.TPieceType.ptLance)),{})) {
        Result = false}
       else Result = !this.IsPathClear(fx,fy,tx,ty);
      return Result;
    };
  });
});
rtl.module("program",["System","SysUtils","Classes","Web","JS","quantum_shogi_logic"],function () {
  "use strict";
  var $mod = this;
  rtl.createClass(this,"TGame",pas.quantum_shogi_logic.TGameLogic,function () {
    this.ValidMoves$a$clone = function (a) {
      var b = [];
      b.length = 9;
      for (var c = 0; c < 9; c++) b[c] = a[c].slice(0);
      return b;
    ...
    this.$init = function () {
      pas.quantum_shogi_logic.TGameLogic.$init.call(this);
      this.CurrentPlayer = 0;
      this.SelectedX = 0;
      this.SelectedY = 0;
      this.SelectedStandIndex = 0;
      this.GameOver = false;
      this.ValidMoves = rtl.arraySetLength(null,false,9,9);
    };
    this.$final = function () {
      this.ValidMoves = undefined;
      pas.quantum_shogi_logic.TGameLogic.$final.call(this);
    };
    this.Create$2 = function () {
      pas.quantum_shogi_logic.TGameLogic.Create$1.call(this);
      this.CurrentPlayer = 0;
      this.SelectedX = -1;
      this.SelectedY = -1;
      this.SelectedStandIndex = -1;
      this.GameOver = false;
      return this;
    };
    this.HandleCellClick = function (ax, ay) {
      if (this.SelectedStandIndex !== -1) {
        if (this.Board[ax - 1][ay - 1] === null) {
          this.DropPiece(this.SelectedStandIndex,ax,ay);
          this.SelectedStandIndex = -1;
        };
        return;
      };
      if (this.SelectedX === -1) {
        if ((this.Board[ax - 1][ay - 1] !== null) && (this.Board[ax - 1][ay - 1].Owner === this.CurrentPlayer)) {
          this.SelectedX = ax;
          this.SelectedY = ay;
          this.UpdateUI();
        };
      ...
      if ((this.SelectedX === ax) && (this.SelectedY === ay)) {
        this.SelectedX = -1;
        this.SelectedY = -1;
        this.UpdateUI();
        return;
      };
      if (this.ValidMoves[ax - 1][ay - 1]) {
        this.MovePiece(this.SelectedX,this.SelectedY,ax,ay);
        this.SelectedX = -1;
        this.SelectedY = -1;
        return;
      };
    ...
    this.HandleStandClick = function (PlayerIdx, PieceIdx) {
      if (PlayerIdx !== this.CurrentPlayer) return;
      this.SelectedStandIndex = PieceIdx;
      this.SelectedX = -1;
      this.UpdateUI();
    };
    this.MovePiece = function (fx, fy, tx, ty) {
      var p = null;
      var newSet = {};
      var pt = 0;
      p = this.Board[fx - 1][fy - 1];
      newSet = {};
      for (var $l in p.Possibilities) {
        pt = +$l;
        if (this.CanMove(pt,tx - fx,ty - fy,p.Owner)) newSet = rtl.includeSet(newSet,pt);
      };
      if (rtl.eqSet(newSet,{})) return;
      if (this.WouldExceedTypeCount(p.Owner,p,rtl.refSet(newSet))) return;
      if (this.IsMovePathBlocked(fx,fy,tx,ty,rtl.refSet(newSet))) return;
      p.Possibilities = rtl.refSet(newSet);
      for (var $l1 in newSet) {
        pt = +$l1;
        if (rtl.eqSet(newSet,rtl.createSet(pt))) {
          if (this.CountPiecesWithTypeInGroup(p.InitialGroup,pt) >= this.InitialCountPerGroup(pt)) this.RemovePossibilityFromGroup(p.InitialGroup,pt,p);
          break;
        };
      };
      if (this.Board[tx - 1][ty - 1] !== null) {
        this.Board[tx - 1][ty - 1].Owner = p.Owner;
        this.Stands[p.Owner].Add(this.Board[tx - 1][ty - 1]);
      };
      this.Board[tx - 1][ty - 1] = p;
      this.Board[fx - 1][fy - 1] = null;
      this.CurrentPlayer = 1 - this.CurrentPlayer;
      this.UpdateUI();
    };
    this.DropPiece = function (PieceIdx, tx, ty) {
      var p = null;
      var newSet = {};
      var pt = 0;
      if (this.Board[tx - 1][ty - 1] !== null) return;
      p = rtl.getObject(this.Stands[this.CurrentPlayer].Get(PieceIdx));
      newSet = rtl.refSet(p.Possibilities);
      if ((this.CurrentPlayer === 0) && (ty === 1)) newSet = rtl.diffSet(newSet,rtl.createSet(pas.quantum_shogi_logic.TPieceType.ptPawn,pas.quantum_shogi_logic.TPieceType.ptLance));
      if ((this.CurrentPlayer === 1) && (ty === 9)) newSet = rtl.diffSet(newSet,rtl.createSet(pas.quantum_shogi_logic.TPieceType.ptPawn,pas.quantum_shogi_logic.TPieceType.ptLance));
      if (rtl.eqSet(newSet,{})) return;
      if (this.WouldExceedTypeCount(this.CurrentPlayer,p,rtl.refSet(newSet))) return;
      p.Possibilities = rtl.refSet(newSet);
      for (var $l in newSet) {
        pt = +$l;
        if (rtl.eqSet(newSet,rtl.createSet(pt))) {
      ...
      this.Board[tx - 1][ty - 1] = p;
      this.Stands[this.CurrentPlayer].Delete(PieceIdx);
      this.CurrentPlayer = 1 - this.CurrentPlayer;
      this.UpdateUI();
    };
    this.CanMove = function (pt, dx, dy, Owner) {
      var Result = false;
      var absX = 0;
      var absY = 0;
      if (Owner === 1) {
      ...
      absY = Math.abs(dy);
      var $tmp = pt;
      if ($tmp === pas.quantum_shogi_logic.TPieceType.ptPawn) {
        Result = (dx === 0) && (dy === -1)}
       else if ($tmp === pas.quantum_shogi_logic.TPieceType.ptLance) {
        Result = (dx === 0) && (dy < 0)}
       else if ($tmp === pas.quantum_shogi_logic.TPieceType.ptKnight) {
        Result = (absX === 1) && (absY === 2) && (dy === -2)}
       else if ($tmp === pas.quantum_shogi_logic.TPieceType.ptSilver) {
        Result = (absX <= 1) && (absY <= 1) && !((dx === 0) && (dy === 0)) && ((dy === -1) || ((dy === 1) && (absX === 1)))}
       else if ($tmp === pas.quantum_shogi_logic.TPieceType.ptKing) {
        Result = (absX <= 1) && (absY <= 1) && !((dx === 0) && (dy === 0))}
       else if ($tmp === pas.quantum_shogi_logic.TPieceType.ptRook) {
        Result = (dx === 0) || (dy === 0)}
       else if ($tmp === pas.quantum_shogi_logic.TPieceType.ptBishop) {
        Result = absX === absY}
       else if ($tmp === pas.quantum_shogi_logic.TPieceType.ptGold) Result = (absX <= 1) && (dy >= -1) && (dy <= 1) && !((dy === 1) && (absX === 1));
      return Result;
    };
    this.ComputeValidMoves = function (fx, fy) {
      var p = null;
      var tx = 0;
      var ty = 0;
      var newSet = {};
      var pt = 0;
      p = this.Board[fx - 1][fy - 1];
      for (ty = 1; ty <= 9; ty++) for (tx = 1; tx <= 9; tx++) {
        if ((tx === fx) && (ty === fy)) {
          this.ValidMoves[tx - 1][ty - 1] = false}
         else if ((this.Board[tx - 1][ty - 1] !== null) && (this.Board[tx - 1][ty - 1].Owner === p.Owner)) {
          this.ValidMoves[tx - 1][ty - 1] = false}
         else {
          newSet = {};
          for (var $l in p.Possibilities) {
          ...
          this.ValidMoves[tx - 1][ty - 1] = rtl.neSet(newSet,{}) && !this.WouldExceedTypeCount(p.Owner,p,rtl.refSet(newSet)) && !this.IsMovePathBlocked(fx,fy,tx,ty,rtl.refSet(newSet));
        };
      };
    };
    this.UpdateUI = function () {
      ...
      var StandEl = null;
      var p = null;
      if ((this.SelectedX !== -1) && (this.SelectedY !== -1) && (this.Board[this.SelectedX - 1][this.SelectedY - 1] !== null)) {
        this.ComputeValidMoves(this.SelectedX,this.SelectedY)}
       else for (y = 1; y <= 9; y++) for (x = 1; x <= 9; x++) this.ValidMoves[x - 1][y - 1] = false;
      for (y = 1; y <= 9; y++) for (x = 1; x <= 9; x++) {
        Cell = document.getElementById("cell-" + pas.SysUtils.IntToStr(x) + "-" + pas.SysUtils.IntToStr(y));
        Container = Cell.querySelector(".piece-container");
        Container.innerHTML = "";
        if ((this.SelectedX === x) && (this.SelectedY === y)) {
          Cell.classList.add("selected")}
         else Cell.classList.remove("selected");
        if ((this.SelectedX !== -1) && (this.SelectedY !== -1) && this.ValidMoves[x - 1][y - 1]) {
          Cell.classList.add("move-dest")}
         else Cell.classList.remove("move-dest");
        if (this.Board[x - 1][y - 1] !== null) this.RenderPieceTo(Container,this.Board[x - 1][y - 1]);
      };
      for (s = 0; s <= 1; s++) {
        StandEl = document.getElementById("stand-" + pas.SysUtils.IntToStr(s));
        StandEl.innerHTML = "";
        for (var $l = 0, $end = this.Stands[s].GetCount() - 1; $l <= $end; $l++) {
          i = $l;
          p = rtl.getObject(this.Stands[s].Get(i));
          Container = document.createElement("div");
          Container.className = "cell";
          if ((s === this.CurrentPlayer) && (this.SelectedStandIndex === i)) Container.classList.add("selected");
          Container.dataset["idx"] = pas.SysUtils.IntToStr(i);
          Container.dataset["owner"] = pas.SysUtils.IntToStr(s);
          Container.onclick = rtl.createSafeCallback($mod,"OnStandClick");
          this.RenderPieceTo(Container,p);
          StandEl.appendChild(Container);
        };
      };
      if (this.CurrentPlayer === 0) {
        document.getElementById("status").innerText = "手番: 先手"}
       else document.getElementById("status").innerText = "手番: 後手";
    };
    this.RenderPieceTo = function (container, p) {
      var pt = 0;
      var Count = 0;
      var PieceChar = "";
      var Span = null;
      ...
      for (var $l1 in p.Possibilities) {
        ...
        if ($tmp === pas.quantum_shogi_logic.TPieceType.ptPawn) {
          ...
          PieceChar = "銀"}
         else if ($tmp === pas.quantum_shogi_logic.TPieceType.ptGold) {
          PieceChar = "金"}
         else if ($tmp === pas.quantum_shogi_logic.TPieceType.ptBishop) {
          PieceChar = "角"}
         else if ($tmp === pas.quantum_shogi_logic.TPieceType.ptRook) {
          PieceChar = "飛"}
         else if ($tmp === pas.quantum_shogi_logic.TPieceType.ptKing) PieceChar = "王";
        Span = document.createElement("span");
        Span.className = "quantum-char";
        Span.innerText = PieceChar;
        Span.style.setProperty("opacity",pas.SysUtils.FloatToStr(1.0 / Count));
        if (p.Owner === 1) {
          Span.style.setProperty("transform","translate(-50%, -50%) rotate(180deg)")}
  ...
  this.OnCellClick = function (e) {
    var Result = false;
    $mod.Game.HandleCellClick(pas.SysUtils.StrToInt(e.currentTarget.dataset["x"]),pas.SysUtils.StrToInt(e.currentTarget.dataset["y"]));
    Result = true;
    return Result;
  };
  this.OnStandClick = function (e) {
    var Result = false;
    $mod.Game.HandleStandClick(pas.SysUtils.StrToInt(e.currentTarget.dataset["owner"]),pas.SysUtils.StrToInt(e.currentTarget.dataset["idx"]));
    Result = true;
    return Result;
  };
  this.RunGroupTwoPieceTest = function () {
    var TestGame = null;
    var p1 = null;
    var p2 = null;
    var p3 = null;
    ...
    var ok = false;
    var msg = "";
    TestGame = $mod.TGame.$create("Create$2");
    for (y = 1; y <= 9; y++) for (x = 1; x <= 9; x++) TestGame.Board[x - 1][y - 1] = null;
    TestGame.Stands[0].Clear();
    TestGame.Stands[1].Clear();
    p1 = pas.quantum_shogi_logic.TQuantumPiece.$create("Create$1",[0,0]);
    p1.Possibilities = rtl.createSet(pas.quantum_shogi_logic.TPieceType.ptLance);
    p2 = pas.quantum_shogi_logic.TQuantumPiece.$create("Create$1",[0,0]);
    p2.Possibilities = rtl.createSet(pas.quantum_shogi_logic.TPieceType.ptLance,pas.quantum_shogi_logic.TPieceType.ptRook);
    p3 = pas.quantum_shogi_logic.TQuantumPiece.$create("Create$1",[0,0]);
    p3.Possibilities = rtl.createSet(pas.quantum_shogi_logic.TPieceType.ptLance,pas.quantum_shogi_logic.TPieceType.ptKnight);
    TestGame.Board[0][4] = p1;
    TestGame.Board[1][4] = p2;
    TestGame.Board[2][4] = p3;
    cnt = TestGame.CountPiecesWithTypeInGroup(0,pas.quantum_shogi_logic.TPieceType.ptLance);
    if (cnt !== 1) {
      msg = "FAIL: 1枚目のみ香確定のとき CountPiecesWithTypeInGroup(0,ptLance)=" + pas.SysUtils.IntToStr(cnt) + " (expected 1)";
      document.getElementById("status").innerText = msg;
      return;
    };
    p2.Possibilities = rtl.createSet(pas.quantum_shogi_logic.TPieceType.ptLance);
    cnt = TestGame.CountPiecesWithTypeInGroup(0,pas.quantum_shogi_logic.TPieceType.ptLance);
    if (cnt < TestGame.InitialCountPerGroup(pas.quantum_shogi_logic.TPieceType.ptLance)) {
      msg = "FAIL: 2枚目確定後 Count=" + pas.SysUtils.IntToStr(cnt);
      document.getElementById("status").innerText = msg;
      return;
    };
    TestGame.RemovePossibilityFromGroup(0,pas.quantum_shogi_logic.TPieceType.ptLance,p2);
    ok = rtl.eqSet(p1.Possibilities,rtl.createSet(pas.quantum_shogi_logic.TPieceType.ptLance)) && rtl.eqSet(p2.Possibilities,rtl.createSet(pas.quantum_shogi_logic.TPieceType.ptLance)) && rtl.eqSet(p3.Possibilities,rtl.createSet(pas.quantum_shogi_logic.TPieceType.ptKnight));
    if (ok) {
      msg = "PASS: 香グループ内2枚テスト — 1枚目・2枚目は香のまま、3枚目から香の可能性が削除されている"}
     else msg = "FAIL: p1=" + pas.SysUtils.IntToStr(pas.quantum_shogi_logic.TPieceType.ptLance) + " p2=" + pas.SysUtils.IntToStr(pas.quantum_shogi_logic.TPieceType.ptLance) + " p3(expect no lance)";
    document.getElementById("status").innerText = msg;
  };
  this.RunGroupTwoPieceTestClick = function (e) {
    var Result = false;
    $mod.RunGroupTwoPieceTest();
    Result = true;
    return Result;
  };
  this.ix = 0;
  ...
  this.BtnEl = null;
  $mod.$main = function () {
    $mod.Game = $mod.TGame.$create("Create$2");
    $mod.BoardEl = document.getElementById("board");
    for ($mod.iy = 1; $mod.iy <= 9; $mod.iy++) for ($mod.ix = 1; $mod.ix <= 9; $mod.ix++) {
      $mod.CellEl = document.createElement("div");
      $mod.CellEl.className = "cell";
      $mod.CellEl.id = "cell-" + pas.SysUtils.IntToStr($mod.ix) + "-" + pas.SysUtils.IntToStr($mod.iy);
      $mod.ContEl = document.createElement("div");
      $mod.ContEl.className = "piece-container";
      $mod.CellEl.appendChild($mod.ContEl);
      $mod.CellEl.dataset["x"] = pas.SysUtils.IntToStr($mod.ix);
      $mod.CellEl.dataset["y"] = pas.SysUtils.IntToStr($mod.iy);
      $mod.CellEl.onclick = rtl.createSafeCallback($mod,"OnCellClick");
      $mod.BoardEl.appendChild($mod.CellEl);
    };
    $mod.Game.UpdateUI();
    $mod.BtnEl = document.createElement("button");
    $mod.BtnEl.id = "test-btn";
    $mod.BtnEl.innerText = "香・桂・金・銀 2枚テスト";
    $mod.BtnEl.onclick = rtl.createSafeCallback($mod,"RunGroupTwoPieceTestClick");
    document.body.appendChild($mod.BtnEl);
  };
});
